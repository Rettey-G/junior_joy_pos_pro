...
const NewInvoice = () => {
  ...
  const handleCreateInvoice = async () => {
    if (cart.length === 0) {
      alert('Please add items to cart before creating invoice');
      return;
    }
    ...
    try {
      ...
      setError('Failed to create invoice: ' + (err.response?.data?.message || err.message));
    }
  };
  ...
  return (
    <div className="sales-container" style={{flexDirection: 'column', gap: '32px', maxWidth: 1200, margin: '0 auto', padding: '24px 8px'}}>
      <h2 className="mb-4" style={{marginBottom: 32, color: '#1976d2'}}>Create New Invoice</h2>
          ...
          <div className="bill-header" style={{marginBottom: 24}}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
              ...
              <img 
                ...
                alt="Junior Joy Logo" 
                style={{width: 70, height: 70, borderRadius: '50%', objectFit: 'cover', border: '2px solid #bbdefb', boxShadow: '0 2px 8px rgba(33,150,243,0.15)'}} 
                onError={(e) => {
